...
class BookingCard extends StatelessWidget {
  ...
  const BookingCard({super.key, required this.booking, this.onEdit,
    this.onCancel,
    this.onRate,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final displayFrom = isUpdate
        ...
        : booking.fromDate;
    final displayTo = isUpdate ? (booking.newToDate ?? booking.toDate) : booking
        .toDate;
    final displayTotal = isUpdate ? (booking.newTotalPrice ??
          ...
          Row(
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: Image.network(
                  booking.property.imageUrls.isNotEmpty ? booking.property.imageUrls.first : '',
                  width: 120,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDetailRow(labelTo, " : ${dateFormat.format(displayTo)}"), // Formatting to match photo "to :"
  ...
  Widget _buildTopActionSection() {
    ...
    if (booking.status == BookingStatus.pending || booking.status == BookingStatus.current) {
      return Row(
    ...
    if (booking.status == BookingStatus.ended) {
      ...
      return Row(
        mainAxisAlignment: MainAxisAlignment.end,
        children: [
          _smallButton("rate", MyColor.deepBlue, onRate),
        ],
      );
    }

    // REJECTED / CANCELED: Show Red Text
    if (booking.status == BookingStatus.rejected || booking.status == BookingStatus.canceled) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(
            value,
            style: const TextStyle(color: MyColor.deepBlue, fontSize: 12, fontWeight: FontWeight.w500),
          ),
        ],
      ),
    );
